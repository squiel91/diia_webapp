...
<script>
	export default {
		...
		methods: {
			emit() {
				this.$emit('change', this.interactionTypes)
			},
			click(interactionType) {
				this.interactionTypes[interactionType] = !this.interactionTypes[interactionType]
				this.emit()
			}
		}
	}
